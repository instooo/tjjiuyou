    ...
    static public function AccessDecision($appName=MODULE_NAME) {
        ...
        if(self::checkAccess()) {
            ...
            if(empty($_SESSION[C('ADMIN_AUTH_KEY')])) {
                ...
                unset($accessList['/']);
                $state = false;
                if (isset($accessList[strtoupper('/'.CONTROLLER_NAME.'/'.ACTION_NAME)])) {
                    $state = true;
                }
                if($state) {
                    ...
                    return true;
                    /*
                    $flag=getKeyExit($accessList['/'][strtoupper('/'.CONTROLLER_NAME.'/'.ACTION_NAME)],strtoupper(ACTION_NAME));
                    if($flag){
                    	$_SESSION[$accessGuid]  =   true;
                    	return true;
                    }else{
    ...
    static public function getAccessList($authId) {
        ...
        foreach($apps as $key=>$app) {
            ...
            foreach($modules as $key=>$module) {
                ...
                foreach ($rs as $a){
                	$actionV1	=	array();
                	$actionV1Name	=	strtoupper($a['name']);
                   	//这里扩充4级菜单
                   	$actionIdV3	=	$a['id'];

                   	$action[$actionV1Name]=$actionIdV3;
                   	$sql    =   "select node.id,node.name from ".
                   			$table['role']." as role,".
                   			$table['user']." as user,".
                   			$table['access']." as access ,".
                   			$table['node']." as node ".
                   			"where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=4 and node.pid={$actionIdV3} and node.status=1";
                   	$rsV4 =   $db->query($sql);
                   	if(!empty($rsV4)){
                   		$action[$actionV1Name]=array();
                   	}
					foreach ($rsV4 as $a4){
						$actionIdV4	=	$a4['id'];
						$actionV2Name=	 strtoupper($a4['name']);
						$action[$actionV1Name][$actionV2Name]=$actionIdV4;
						//这里扩充5级菜单
						$sql    =   "select node.id,node.name from ".
								$table['role']." as role,".
								$table['user']." as user,".
								$table['access']." as access ,".
								$table['node']." as node ".
								"where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=5 and node.pid={$actionIdV4} and node.status=1";
						$rsV5 =   $db->query($sql);
						if(!empty($rsV5)){
							$action[$actionV1Name][$actionV2Name]=array();
						}
						foreach ($rsV5 as $a5){
							$actionIdV5	=	$a5['id'];
							$actionV3Name=	 strtoupper($a5['name']);
							$action[$actionV1Name][$actionV2Name][$actionV3Name]=$actionIdV5;
						}	
