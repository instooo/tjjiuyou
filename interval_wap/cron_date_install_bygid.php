...
define('API', true);
define('LOG_FILE', 'log_install_bygid.log');
...
if (count($flag) > 0) {
    //重复统计，删除原有数据
    $dele_oldData_sql = "DELETE FROM `mygame_tj_install_bygid` WHERE `date`={$t_date}";
...
$flag_find_sql = "SELECT * FROM `mygame_tj_install_bygid` WHERE  `date`={$t_date} LIMIT 1";
...
$member_sql ="SELECT COUNT(id) install_num,gid,channel AS sub_channels FROM mygame_install WHERE install_time  BETWEEN {$t_date_start} AND {$t_date_end} GROUP BY gid,channel";
...
if($mlog){
    ...
    foreach($mlog as $key=>$val){
        ...
        //日期,(安装数,游戏,媒体),一级媒体,添加时间
        //date,install_num,gid,sub_channels,total_channels,addtime
    ...
    $values = trim($values, ',');
    if($values){
        $insert_sql ="INSERT INTO `mygame_tj_install_bygid` (`date`,`install_num`,`gid`,`sub_channels`,`total_channels`,`addtime`) VALUES {$values}";
