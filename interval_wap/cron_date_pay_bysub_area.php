...
define('API', true);
define('LOG_FILE', 'log_pay_bysub_area.log');
...
if (count($flag) > 0) {
    //重复统计，删除原有数据
    $dele_oldData_sql = "DELETE FROM `mygame_tj_pay_bysub_area` WHERE `date`={$t_date}";
...
$flag_find_sql = "SELECT * FROM `mygame_tj_pay_bysub_area` WHERE  `date`={$t_date} LIMIT 1";
...
$member_sql ="SELECT SUM(pay_money) as now_money,count(distinct(uid)) as spend_num,count(distinct(CASE WHEN FROM_UNIXTIME(register_time,'%Y%m%d')={$t_date} THEN uid END)) as dan_num, COUNT(uid) as spend_tnum ,COUNT(CASE WHEN FROM_UNIXTIME(register_time,'%Y%m%d')={$t_date} THEN 1 END) as dan_tnum,channel as sub_channels,province FROM mygame_pay_log_area WHERE pay_time BETWEEN {$t_date_start} AND {$t_date_end} GROUP BY channel,province";
...
if($mlog){
    ...
    foreach($mlog as $key=>$val){
        ...
        //日期,总金额,(当期充值,今日充值人数,当日注册且充值人数,今日充值人次,当日注册且充值人次,游戏,媒体,地区),一级媒体,添加时间
        //date,total_money,(now_money,spend_num,dan_num,spend_tnum,dan_tnum,sub_channels),total_channels,addtime
        $values .= "({$t_date},{$total_money},{$str},{$totalchanne[$val['sub_channels']]},'{$province}',{$time}),";
    }
    unset($mlog); unset($totalchanne); //清内存
    $values = trim($values, ',');

    if($values){
        $insert_sql ="INSERT INTO `mygame_tj_pay_bysub_area` (`date`,`total_money`,`now_money`,`spend_num`,`dan_num`,`spend_tnum`,`dan_tnum`,`sub_channels`,`total_channels`,`area`,`addtime`) VALUES {$values}";
