...
define('API', true);
define('LOG_FILE', 'log_member_byarea.log');
...
if (count($flag) > 0) {
    //重复统计，删除原有数据
    $dele_oldData_sql = "DELETE FROM `mygame_tj_member_byarea` WHERE `date`={$t_date}";
...
$flag_find_sql = "SELECT * FROM `mygame_tj_member_byarea` WHERE  `date`={$t_date} LIMIT 1";
...
$member_sql ="SELECT province,COUNT(uid) as register_num,gid,channel as sub_channels FROM mygame_member_area WHERE register_time BETWEEN {$t_date_start} AND {$t_date_end} GROUP BY gid,channel,province";
...
if($mlog){
    ...
    foreach($mlog as $key=>$val){
        ...
        //(日期),地区,注册数,游戏,媒体,一级媒体,添加时间
        $values .= "({$t_date},'{$province}',{$str},{$totalchanne[$val['sub_channels']]},{$time}),";
    }
    unset($mlog); unset($totalchanne); //清内存
    $values = trim($values, ',');

    if($values){
        $insert_sql ="INSERT INTO `mygame_tj_member_byarea` (`date`,`area`, `register_num`, `gid`, `sub_channels`, `total_channels`,`addtime`) VALUES {$values}";
