...
define('API', true);
define('LOG_FILE', 'log_pay_bysub_hour.log');
...
if (count($flag) > 0) {
    //重复统计，删除原有数据
    $dele_oldData_sql = "DELETE FROM `mygame_tj_pay_bysub_hour` WHERE `date`={$t_date}";
...
$flag_find_sql = "SELECT * FROM `mygame_tj_pay_bysub_hour` WHERE  `date`={$t_date} LIMIT 1";
...
$member_sql ="SELECT SUM(pay_money) as now_money,count(distinct(uid)) as spend_num,count(distinct(CASE WHEN FROM_UNIXTIME(register_time,'%Y%m%d')={$t_date} THEN uid END)) as dan_num, COUNT(uid) as spend_tnum ,COUNT(CASE WHEN FROM_UNIXTIME(register_time,'%Y%m%d')={$t_date} THEN 1 END) as dan_tnum,channel as sub_channels,FROM_UNIXTIME(pay_time, '%H') as hour FROM mygame_pay_log WHERE pay_time BETWEEN {$t_date_start} AND {$t_date_end} GROUP BY channel,hour";
...
if($mlog){
    ...
    foreach($newslog as $key=>$val){
        ...
        $values .= "({$t_date},'{$val['total_money']}','{$val['now_money']}','{$val['spend_num']}','{$val['dan_num']}','{$val['spend_tnum']}','{$val['dan_tnum']}',{$val['sub_channels']},{$val['total_channels']},{$time}),";
    }
    $values = trim($values, ',');

    if($values){
        $insert_sql ="INSERT INTO `mygame_tj_pay_bysub_hour` (`date`,`total_money`,`now_money`,`spend_num`,`dan_num`,`spend_tnum`,`dan_tnum`,`sub_channels`,`total_channels`,`addtime`) VALUES {$values}";
