...
define('API', true);
define('LOG_FILE', 'log_pay_bysub.log');
...
if (count($flag) > 0) {
    //重复统计，删除原有数据
    $dele_oldData_sql = "DELETE FROM `mygame_tj_pay_bysub` WHERE `date`={$t_date}";
...
$flag_find_sql = "SELECT * FROM `mygame_tj_pay_bysub` WHERE  `date`={$t_date} LIMIT 1";
...
$member_sql ="SELECT SUM(pay_money) as now_money,count(distinct(uid)) as spend_num,count(distinct(CASE WHEN FROM_UNIXTIME(register_time,'%Y%m%d')={$t_date} THEN uid END)) as dan_num, COUNT(uid) as spend_tnum ,COUNT(CASE WHEN FROM_UNIXTIME(register_time,'%Y%m%d')={$t_date} THEN 1 END) as dan_tnum,channel as sub_channels FROM mygame_pay_log WHERE pay_time BETWEEN {$t_date_start} AND {$t_date_end} GROUP BY channel";
...
if($mlog){
    ...
    if($values){
        $insert_sql ="INSERT INTO `mygame_tj_pay_bysub` (`date`,`total_money`,`now_money`,`spend_num`,`dan_num`,`spend_tnum`,`dan_tnum`,`sub_channels`,`total_channels`,`addtime`) VALUES {$values}";
